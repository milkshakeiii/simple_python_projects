...
class ChallengeClient():
    ...
    def run(self) -> None:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
            ...
            request_thread = threading.Thread(target=self.send_requests, args=(sock, exit_event))
    ...
    def receive_responses(self, sock: socket.socket, exit_event: threading.Event) -> None:
        unfinished_response = ""
        while not exit_event.is_set():
    ...
    def do_scan(self, exit_event: threading.Event) -> int:
        ...
        while len(unanswered_dirlists) > 0 and not exit_event.is_set():
            if not self.server_responses.empty():
                ...
                if responses_processed in [10, 1000] or responses_processed%10000 == 0:
